...
use super::{
    packetid, Connection, ConnectionSnapshot, DataRequest, Event, FilterIdx, Meter, Notification,
    Print, RouterMeter, ShadowRequest, SubscriptionSnapshot, MAX_CHANNEL_CAPACITY,
...
impl Router {
            ...
            match packet {
                ...
                Packet::Subscribe(mut subscribe, props) => {
                    ...
                    for f in &mut subscribe.filters {
                        ...
                        if let Err(e) = validate_subscription(connection, f, &self.config.acl_handler) {
    ...
    fn send_meters(&mut self) {
        ...
        let connections: Vec<ConnectionSnapshot> = self
            .connections
            .iter()
            .filter(|(_, connection)| !connection.bypass_acl)
            ...
            .collect();
        meters.push(Meter::Connections(connections));

        let subscriptions: Vec<SubscriptionSnapshot> = self
            .subscription_map
            .iter()
            .filter_map(|(filter, connection_ids)| {
                let subscribers: Vec<String> = connection_ids
                    .iter()
                    .filter_map(|id| self.connections.get(*id))
                    .filter(|connection| !connection.bypass_acl)
            ...
            .collect();
        meters.push(Meter::Subscriptions(subscriptions));
...
fn append_to_commitlog(
    ...
    check_acl(connection, topic, AclOperation::Publish, acl_handler)?;
...
fn validate_subscription(
    ...
    check_acl(connection, &filter.path, AclOperation::Subscribe, acl_handler)?;
...
fn check_acl(
    ...
    if connection.bypass_acl {
        return Ok(());
    }

    if let Some(handler) = acl_handler {
        let username = connection.username.as_deref().unwrap_or("");
        if !handler(username, topic_or_filter, op) {
            return Err(RouterError::AclDenied(topic_or_filter.to_owned()));
        }
    }
