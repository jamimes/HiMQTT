...
pub use link::meters;
pub use router::{
    Alert, ConnectionSnapshot, Forward, IncomingMeter, Meter, Notification, OutgoingMeter, Router,
...
pub type AclHandler = Arc<dyn Fn(&str, &str, AclOperation) -> bool + Send + Sync>;
...
impl Config {
    ...
    where
        F: Fn(&str, &str, AclOperation) -> bool + Send + Sync + 'static,
    {
        self.router.set_acl_handler(acl_fn);
    }
}
...
pub struct RouterConfig {
    ...
    pub acl_handler: Option<AclHandler>,
}
...
impl RouterConfig {
    ...
    where
        F: Fn(&str, &str, AclOperation) -> bool + Send + Sync + 'static,
    {
        self.acl_handler = Some(Arc::new(acl_fn));
    }
}
...
impl fmt::Debug for RouterConfig {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("RouterConfig")
            ...
            .field("max_outgoing_packet_count", &self.max_outgoing_packet_count)
            .field("max_segment_size", &self.max_segment_size)
            .field("max_segment_count", &self.max_segment_count)
            .field("custom_segment", &self.custom_segment)
            .field("initialized_filters", &self.initialized_filters)
            .field(
                "shared_subscriptions_strategy",
                &self.shared_subscriptions_strategy,
            )
            .field("acl_handler", &self.acl_handler.is_some())
            .finish()
    }
}
...
impl<'de> Deserialize<'de> for RouterConfig {
    ...
    {
        #[derive(Deserialize)]
        struct RouterConfigData {
            max_connections: usize,
            max_outgoing_packet_count: u64,
            max_segment_size: usize,
            max_segment_count: usize,
            custom_segment: Option<HashMap<String, SegmentConfig>>,
            initialized_filters: Option<Vec<Filter>>,
            #[serde(default)]
            shared_subscriptions_strategy: Strategy,
        }

        let data = RouterConfigData::deserialize(deserializer)?;
        Ok(Self {
            max_connections: data.max_connections,
            max_outgoing_packet_count: data.max_outgoing_packet_count,
            max_segment_size: data.max_segment_size,
            max_segment_count: data.max_segment_count,
            custom_segment: data.custom_segment,
            initialized_filters: data.initialized_filters,
            shared_subscriptions_strategy: data.shared_subscriptions_strategy,
            acl_handler: None,
        })
    }
}
...
impl Serialize for RouterConfig {
    ...
    {
        #[derive(Serialize)]
        struct RouterConfigData<'a> {
            max_connections: usize,
            max_outgoing_packet_count: u64,
            max_segment_size: usize,
            max_segment_count: usize,
            custom_segment: &'a Option<HashMap<String, SegmentConfig>>,
            initialized_filters: &'a Option<Vec<Filter>>,
            shared_subscriptions_strategy: Strategy,
        }
        ...
        RouterConfigData {
            ...
            initialized_filters: &self.initialized_filters,
            shared_subscriptions_strategy: self.shared_subscriptions_strategy.clone(),
        }
